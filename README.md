...
| 単一誤差 | 1ポイントの偏差の絶対値の最大 [秒] |
| 隣接誤差 | 隣接ポイント間の偏差差の最大値 [秒] |
...
| バックラッシ 判定 | **測定温度に応じた規格**との突き合わせで OK / NG。ホイールが合金製のため熱膨張でバックラッシが変わり、温度帯ごとに規格が異なる |
| 真の最大・最小 | ホイール偏差とウォーム偏差が最悪方向に重なった合成値（仮実装、下記参照） |

### バックラッシの温度別規格

`settings.json` の `backlash_spec` で温度帯ごとに規格を設定する:
...
- **バックラッシの温度別規格値**: `backlash_spec` の初期値は仮置き。
  実際の規格表をもらえれば確定できる。
