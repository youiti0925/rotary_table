...
DEFAULTS = dict(
    ...
    param_master_prm="",      # FANUC: 直接指定するBASIC .prm（フォルダ運用なら空でOK）
    ...
    # 機械へ渡すパラメータファイルの拡張子。実機が自分で出力するのが .DAT で、
    # そちらは読み込めることを確認済み（.prm は読めなかった）。
    param_out_ext=".DAT",
    # 一括作成で出す測定プログラムの拡張子
    nc_out_ext=".NC",
    # 制御装置マスタ（号機・CNC・軸ごとの容量等）。必要容量で制御装置を絞り込むのに使う
    controller_master_csv=r"マスタ/制御装置マスタ.csv",
    # モーター→容量 対応表（例外の上書き用）。通常は容量をモーター型式の番手から標準組合せで
    # 自動判定する（αiS2/4→20A, 8/12→40A, 22/30→80A, 40/50→160A）。標準と違うアンプを
    # 使う場合だけ、この表に型式/番号/2020を書いて容量を上書きする。
    motor_capacity_csv=r"マスタ/モーター容量マスタ.csv",
    # クローズドループ判定（DB表示用）。既定は FANUC 1815 #1(OPTx=別置検出器)=1 → フルクロ、
    # 0 → セミクロ。機種でビット/番号が違うことがあるので変更可（生値表示と併用し人が確認）。
    closed_loop_number="1815",
    closed_loop_bit=1,
    closed_loop_full=1,
    # 作成時オプション: モーター番号変更時にデジタルサーボを再初期化するための
    # DGPRビット（FANUC: 2000 #1 を 0 にする→電源再投入で自動的に1へ）と、
    # 「原点確立」ON/OFF するパラメータ・ビット（既定 1815 の #5 APZ=原点確立済み）。
    # ★機種でビットが違う場合があるので変更可。作成前プレビューで旧→新を必ず確認する。
    motor_zero_param="2000",
    motor_zero_bit=1,            # DGPR（B-65270 で確認）
    origin_param="1815",
    origin_bit=5,
    # 出荷するファイルで0にしておく項目（実機で「ゼロで大丈夫」と確認）。
    # 1850 グリッドシフト / 1851 バックラッシ補正量 / 1852 早送り時バックラッシ。
    # いずれも機械個体の実測値なので、前の機械の値を持ち込まない。
    zero_individual=True,
    zero_individual_params=["1850", "1851", "1852"],
    # 客先パラメータのソフトリミット（1320 ＋側 / 1321 −側）をどうするか。
    # 検査では入れたくないことがあるので作成画面で選ぶ。ここは画面の初期値。
    #   apply=客先どおり入れる / skip=入れない(BASICのまま) / disable=無効化(-1/+1)
    soft_limit_mode="apply",
    soft_limit_params=["1320", "1321"],
    # 出力ファイル名に 型式・軸・傾斜/回転 を付ける（T50013078_RTT-135_Z-TILT.DAT）。
    # 制御装置側で名前が長すぎて困るときは false で <頭文字><Seiban> だけに戻せる。
    param_name_detail=True,
    # サーボアンプ容量 → N2165(アンプ最大電流) の対応（BASICと号機マスタの照合用）
    amp_current_map={"20A": "25", "40A": "45", "80A": "85", "160A": "165"},
